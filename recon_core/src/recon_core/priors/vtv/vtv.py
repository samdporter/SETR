...
from recon_core.priors.vtv.numerical_constants import get_division_epsilon, get_sqrt_epsilon
from recon_core.utils import BlockDataContainerToArray
from recon_core.utils.sirf import get_array


def _smoothing_derivatives(smoothing: str):
    """Return (g', g'') for the configured smoothing potential."""
...
def _directional_projector_stats_from_jacobian(jacobian, like, n_params: int, n_dirs: int):
    """Return (diag(D^T D), ||D||_2^2) per voxel for directional gradients."""
    ...
    def _stats_for_grad(g):
        if g is None or not hasattr(g, "anatomical_grad"):
            diag = torch.ones((*like.shape[:-2], n_dirs), device=like.device, dtype=like.dtype)
            norm_sq = torch.ones((*like.shape[:-2],), device=like.device, dtype=like.dtype)
            return diag, norm_sq
        ...
        xi = anat / torch.sqrt(den**2 + eta**2)
        xi_norm_sq = torch.sum(xi * xi, dim=-1)
        gamma = g.gamma if hasattr(g, "gamma") else 1.0
        gamma = torch.as_tensor(gamma, device=like.device, dtype=like.dtype)
        factor = gamma * (2.0 - gamma * xi_norm_sq)
        diag = 1.0 - factor.unsqueeze(-1) * (xi * xi)
        diag = torch.clamp(diag, min=0.0)
        eig = 1.0 - gamma * xi_norm_sq
        norm = torch.maximum(torch.ones_like(eig), torch.abs(eig))
        norm_sq = norm * norm
        return diag, norm_sq
    ...
    for idx in range(n_params):
        ...
        diag, norm_sq = _stats_for_grad(g)
        diag_list.append(diag)
        norm_list.append(norm_sq)

    diag_all = torch.stack(diag_list, dim=-2)  # (..., M, d)
    norm_all = torch.stack(norm_list, dim=-1)  # (..., M)
    return diag_all, norm_all
...
def _direction_valid_mask(shape, directions, device, dtype, bnd_cond):
    ...
    for dx, dy, dz in directions:
        ...
        my = (ys + dy >= 0) & (ys + dy < ny)
        mz = (zs + dz >= 0) & (zs + dz < nz)
        mask = mx[:, None, None] & my[None, :, None] & mz[None, None, :]
        masks.append(mask)
...
def _shift_with_clamp(x, sh: tuple[int, int, int]):
    dx, dy, dz = (int(v) for v in sh)
    nx, ny, nz = x.shape[:3]
...
def _accumulate_over_neighborhood_block(term_block, directions, bnd_cond):
    """Accumulate 2x2 block contributions over local neighborhood channels."""
...
class WeightedVectorialTotalVariation(Function):
    ...
    def __init__(
        ...
        if self.precond_method not in self._PRECOND_METHODS:
            raise ValueError(
                ...
                f"Options: {sorted(self._PRECOND_METHODS)}"
            )
    ...
    def preconditioner_block(self, x, out=None, eta: float = 0.7, epsilon: float = 1e-8):
        """
    ...
    def inv_preconditioner_block(self, x, out=None, eta: float = 0.7, epsilon: float = 1e-8):
        """
        ...
        p22 = blocks[..., 1, 1]
        det = p11 * p22 - p12 * p12
        eps_det = torch.as_tensor(epsilon, device=det.device, dtype=det.dtype)
        det = torch.clamp(det, min=eps_det * eps_det)
        inv = torch.stack(
...
class TotalVariation(Function):
    ...
    def __init__(
        ...
        """
        from recon_core.core.gradients import DirectionalGradient, Gradient
