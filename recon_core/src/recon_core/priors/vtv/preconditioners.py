...
- The Frobenius surrogate uses a cheap isotropic proxy (see frob_diag docstring).

If you need legacy methods (mm_diag_jensen, mm_block_2x2, ls_block_gershgorin, etc.),
see preconditioners_old.py.
"""
...
_DIAG_METHODS = {"mm_diag", "mm_diag_gershgorin", "frob_diag"}
...
def _directional_scaling_factors(wvtv, x_arr, like):
    base_grad = wvtv.jacobian.grad[0] if isinstance(wvtv.jacobian.grad, list) else wvtv.jacobian.grad
    base_grad = getattr(base_grad, "gradient", base_grad)
    ...
    if len(directions) == 0:
        return None, None, directions, bnd_cond
    ...
    s1 = q_scale[..., 0, :]
    s2 = q_scale[..., 1, :]
    d1 = torch.clamp(d_diag[..., 0, :], min=0.0)
    d2 = torch.clamp(d_diag[..., 1, :], min=0.0)

    scale1 = (w0 * s1) * torch.sqrt(d1)
    scale2 = (w1 * s2) * torch.sqrt(d2)
    return scale1, scale2, directions, bnd_cond


def _map_edge_blocks_to_voxels(wvtv, x_arr, like, edge_blocks, epsilon: float):
    scale1, scale2, directions, bnd_cond = _directional_scaling_factors(wvtv, x_arr, like)
    if scale1 is None:
    ...
    # Broadcast edge_blocks over directions when needed.
    if edge_blocks.ndim == scale1.ndim + 1:
        edge_blocks = edge_blocks.unsqueeze(-3).expand(*scale1.shape, 2, 2)

    w11 = edge_blocks[..., 0, 0]
    w22 = edge_blocks[..., 1, 1]
    w12 = 0.5 * (edge_blocks[..., 0, 1] + edge_blocks[..., 1, 0])

    p11 = (scale1 * scale1) * w11
    p22 = (scale2 * scale2) * w22
    p12 = (scale1 * scale2) * w12

    p_dir = torch.stack(
        [torch.stack([p11, p12], dim=-1), torch.stack([p12, p22], dim=-1)], dim=-2
    )

    p_block = _accumulate_over_neighborhood_block(p_dir, directions, bnd_cond)
...
def compute_precond_diag(wvtv, x_arr, method: str, epsilon: float = 1e-8):
    ...
    - mm_diag: uses only the diagonal of the MM block weight (can underestimate curvature).
    - mm_diag_gershgorin: conservative diagonal majoriser via Gershgorin row sums.
    - frob_diag: cheap isotropic Frobenius surrogate (not nuclear-norm exact).
    """
    ...
    if method in {"mm_diag", "mm_diag_gershgorin"}:
    ...
    if method == "frob_diag":
        # Cheap isotropic proxy: omega = 1 / sqrt(0.5 * ||A||_F^2 + eps^2)
        A_frob_sq = torch.sum(A * A, dim=(-2, -1))
        eps_t = torch.as_tensor(float(wvtv.vtv.eps), device=A.device, dtype=A.dtype)
        omega = torch.reciprocal(torch.sqrt(0.5 * A_frob_sq + eps_t * eps_t))
        P_diag = omega.unsqueeze(-1) * S_jm * w2
        P_diag = torch.nan_to_num(P_diag, nan=0.0, posinf=0.0, neginf=0.0)
        return torch.clamp(P_diag, min=epsilon)
...
def compute_precond_block(wvtv, x_arr, method: str, epsilon: float = 1e-8):
    ...
    - mm_block_diag: MM/IRLS block weight pulled back to voxels.
    - ls_block_diag: LS/dilation-based block curvature pulled back to voxels.
    """
