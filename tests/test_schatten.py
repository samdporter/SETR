...
from setr.priors.vtv.schatten_norm_gpu import GPUVectorialTotalVariation as VTV_SVDFREE
from setr.core.gradients import Jacobian  # your Jacobian class
...
def run_tests():
    ...
    for kind in ["cells3d", "kidney"]:
        ...
        vtv_slow   = VTV_SLOW  (eps=delta, norm=norm, smoothing_function=smoothing, tail=tail, numpy_out=False)
        vtv_stable = VTV_STABLE(eps=delta, norm=norm, smoothing_function=smoothing, tail=tail, numpy_out=False)
        vtv_svdfr  = VTV_SVDFREE(eps=delta, norm=norm, smoothing_function=smoothing, tail=tail, numpy_out=False)
        ...
        print(f"SLOW     | direct: {t_dir_slow:.6f} | gradient: {t_grad_slow:.6f} | proximal: {t_prox_slow:.6f} | hessian: {t_hess_slow:.6f}")
        ...
        print(f"STABLE   | direct: {t_dir_stb:.6f} | gradient: {t_grad_stb:.6f} | proximal: {t_prox_stb:.6f} | hessian: {t_hess_stb:.6f}")
        ...
        if d_dirs in (2,3):
            ...
            print(f"SVD-FREE | direct: {t_dir_svd:.6f} | gradient: {t_grad_svd:.6f} | proximal: {t_prox_svd:.6f} | hessian: {t_hess_svd:.6f}")
        else:
