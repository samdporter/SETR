...
def plot_objective_vs_params(df, output_dir):
    ...
    for precond, color in zip(precond_types, colors):
        ...
        ax.plot(
            best_per_alpha.index,
            best_per_alpha.values,
            marker="o",
            label=precond,
            color=color,
            linewidth=2,
...
def plot_heatmaps(df, output_dir):
    ...
    for ax, precond in zip(axes, precond_types):
        ...
        pivot = precond_df.pivot_table(
            values="final_objective", index="alpha", columns="step_size", aggfunc="min"
        )
        ...
        sns.heatmap(
            ...
            cmap="viridis_r",
            ax=ax,
            cbar_kws={"label": "Final Objective"},
        )
...
def plot_convergence(df, output_dir):
    ...
    for precond, color in zip(precond_types, colors):
        ...
        ax.plot(
            conv_per_alpha.index,
            conv_per_alpha.values,
            marker="o",
            label=precond,
            color=color,
            linewidth=2,
...
def main():
    ...
    parser.add_argument(
        "--results",
        type=str,
        default="results/preconditioner_tests/results.csv",
        help="Path to results CSV file",
    )
    parser.add_argument(
        "--output", type=str, default=None, help="Output directory (default: same as results)"
    )
