...
def create_test_images(initial_estimates):
    ...
    # 1. Initial images (most important)
    ...
    # Central ellipsoid
    pet_ellipsoid = create_ellipsoid_phantom(
        initial_estimates.containers[0], 
        intensity=pet_max
    )
    spect_ellipsoid = create_ellipsoid_phantom(
...
def evaluate_functions(test_image, data_fidelity_funs, prior_funs):
    ...
    # Evaluate individual data fidelity functions
...
def main(args):
    ...
    for name, test_img in test_images.items():
        ...
        plot_coronal_slice(test_img, f"Test Image: {name}", 
    ...
    with open(os.path.join(args.output_path, "function_summary.txt"), "w") as f:
        ...
        for test_name, results in all_results.items():
            ...
            f.write(f"Data Fidelity Gradient Norm: {results['data_fidelity_total']['gradient_norm']:.6e}\n")
