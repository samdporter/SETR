...
Tests 3 preconditioners:
1. BSREM only (no prior preconditioning)
2. BSREM + VTV fast diagonal Hessian preconditioner
3. BSREM + VTV slow diagonal Hessian preconditioner
...
from cil.optimisation.operators import BlockOperator, CompositionOperator, IdentityOperator, ZeroOperator
...
def setup_reconstruction(args):
    ...
    cyl, gauss = get_filters(fwhms=(10, 10, 10))
    ...
    def get_pet_am_with_res():
        return get_pet_am(not args.no_gpu, gauss_fwhm=args.pet_gauss_fwhm)
    ...
    if not os.path.exists(os.path.join(args.output, 'kappas')):
        os.makedirs(os.path.join(args.output, 'kappas'))
    kappas.containers[0].write(os.path.join(args.output, 'kappas', 'pet_kappa_sq.hv'))
    kappas.containers[1].write(os.path.join(args.output, 'kappas', 'spect_kappa_sq.hv'))
...
def create_priors_for_alpha(
    ...
    Args:
        ...
        hessian_type: 'fast' or 'slow' - affects preconditioner only
    ...
    # This ensures cross-modal gradient magnitudes are balanced
...
def create_preconditioner(
    ...
    Args:
        precond_type: 'bsrem', 'vtv_fast', or 'vtv_slow'
    ...
    bsrem_precond = BSREMPreconditioner(
        s_inv, 1, np.inf, epsilon=epsilon, smooth=True,
    )
    ...
    prior_precond = [
        ImageFunctionPreconditioner(
            p.function.inv_hessian_diag,  # Uses the hessian type set in VTV
            1,
            freeze_iter=np.inf,
            epsilon=0,
        )
...
def run_single_test(
    ...
    callbacks = [
        ...
        SavePreconditionerCallback(str(run_output_dir / "preconditioner"), len(setup_data['all_funs'])),
    ]
...
def main():
    ...
    parser.add_argument('--precond-types', type=str, nargs='+',
                       default=['bsrem', 'vtv_fast', 'vtv_slow'])
    ...
    setup_data = setup_reconstruction(args)
    ...
    for run_id, (precond_type, alpha, step_size) in enumerate(param_grid, start=1):
        ...
        hessian_type = 'slow' if precond_type == 'vtv_slow' else 'fast'
