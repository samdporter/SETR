...
for bnd_cond, stencil, both_dir in itertools.product(boundary_conditions, stencils, both_directions_opts):
    ...
    print(f"{config}  Grad:{status_grad}({err_grad:.1e})  Opt:{status_opt}({err_opt:.1e})  Match:{match_str}")
