...
# ================================
# MANC Bootstrap HKEM - PET Reconstruction
# ================================
# This script runs PET HKEM reconstruction with SPECT guidance for each bootstrap
...
log_with_timestamp "Starting MANC PET HKEM reconstruction: Task ${TASK_ID}"
log_with_timestamp "Host: $HOSTNAME"
...
# --- Paths for this job ---
OUTPUT_DIR="$RESULTS_BASE/bootstrap_$(printf '%03d' $BOOTSTRAP_NUM)"
...
# Verify resampled SPECT exists
SPECT_FILE="${BOOTSTRAP_PATH}/spect.hv"
if [ ! -f "$SPECT_FILE" ]; then
    report_failure 1 "Resampled SPECT file not found: $SPECT_FILE" "missing_input"
    exit 1
fi

log_with_timestamp "Using resampled SPECT: $SPECT_FILE"

# Config file
BASE_CONFIG="$CONFIG_DIR/config_manc_hkem_pet.yaml"
if [ ! -f "$BASE_CONFIG" ]; then
    report_failure 1 "Base config file not found: $BASE_CONFIG" "config"
    exit 1
fi

CONFIG_FILE="$CONFIG_DIR/.tmp_manc_hkem_pet_$(printf '%03d' $BOOTSTRAP_NUM).yaml"
cp "$BASE_CONFIG" "$CONFIG_FILE"

# --- Edit YAML config ---
log_with_timestamp "Updating configuration file..."
if ! python3 - <<'PY' "$CONFIG_FILE" "$BOOTSTRAP_PATH" "$OUTPUT_DIR"
import sys, os, tempfile, re, yaml

cfg_file = sys.argv[1]
pet_data_path = sys.argv[2]
...
try:
    ...
    content = re.sub(r'^pet_data_path:\s*["\']?.*["\']?$', f'pet_data_path: "{pet_data_path}"', content, flags=re.MULTILINE)
...
if [ $RETURN_CODE -eq 0 ]; then
    ...
    cat > "$OUTPUT_DIR/job_completion.txt" <<EOF
bootstrap=$BOOTSTRAP_ID,status=completed,end_time=$END_TIME,host=$HOSTNAME,start_time=$START_TIME,output_files=$output_files_count
EOF
